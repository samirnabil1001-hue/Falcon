<form action="{{ url()->current() }}" method="GET"
    class="m-6 mb-0 bg-slate-50/80 dark:bg-slate-800/40 rounded-xl p-4 border border-slate-100 dark:border-slate-800">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="flex gap-2 justify-end">
            ...
            <button type="submit"
                class="bg-slate-200 hover:bg-violet-600 hover:text-white dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-violet-600 text-slate-700 text-xs font-semibold py-2.5 px-5 rounded-xl transition-all shadow-sm">
                تطبيق الفلاتر
            </button>
        </div>
    </div>
