<div x-show="showConfirmedModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;"
    aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div x-show="showConfirmedModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
        ...
        <div x-show="showConfirmedModal" x-transition:enter="ease-out duration-300"
            ...
            <button type="button" @click="showConfirmedModal = false"
            ...
            <form action="{{ $route }}" method="POST" class="m-0">
                                ...
                                <div>
                                    ...
                                    <div class="relative w-full">
                                        <select name="service_type" required
                                            ...
                                            <option value="" disabled selected>اختر نوع الخدمه...</option>
                                            @foreach (\App\Enums\CompanyService::cases() as $service)
                ...
                <div class="bg-gray-50 dark:bg-slate-700/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                    <button type="submit"
                        class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 hover:bg-green-700 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:w-auto sm:text-sm transition-colors">
                        تنفيذ الخدمه
                    </button>
                    <button type="button" @click="showConfirmedModal = false"
